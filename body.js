...
// Enhanced in-memory store implementation
class CustomStore {
    constructor() {
        this.messages = new Map();
        this.contacts = new Map();
        this.chats = new Map();
        this.maxMessages = 10000; // Limit per chat
        this.maxChats = 5000; // Total chats to store
    ...
    cleanup() {
        try {
            // Clean up old chats if we have too many
...
let zk;
let store; // Will be initialized in main function
...
async function main() {
    try {
        ...
        if (store) {
            store.destroy(); // Clean up previous store
        }
        ...
        adams.ev.on("messages.upsert", async ({ messages }) => {
            try {
                ...
                // Store message with timestamp - Enhanced with better memory management
                ...
                // Cleanup old messages (keep only last 50 messages per chat to prevent memory issues)
        ...
        adams.ev.on("connection.update", async (update) => {
            ...
            if (connection === "connecting") {
                ...
                reconnectAttempts = 0; // Reset on new connection attempt
            }

            if (connection === "open") {
                console.log("🌎 BWM XMD ONLINE 🌎");
                reconnectAttempts = 0; // Reset successful connection
...
async function reconnectWithRetry() {
    ...
    reconnectAttempts++;
    const delay = Math.min(RECONNECT_DELAY * Math.pow(2, reconnectAttempts - 1), 300000); // Max 5 minutes

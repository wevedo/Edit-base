...
// ENHANCED SECURITY STORE WITH STRICT AUTHORIZATION
class CustomStore {
    constructor() {
...
async function authentification() {
    try {
        if (!fs.existsSync(__dirname + "/bwmxmd/creds.json")) {
            console.log("Setting up BWM XMD session...");
...
async function main() {
    try {
        ...
        // MAIN COMMAND PROCESSING WITH STRICT AUTHORIZATION
        adams.ev.on("messages.upsert", async ({ messages }) => {
            const ms = messages[0];
            ...
            if (verifCom && com) {
                ...
                if (cmd) {
                    // CRITICAL: STRICT AUTHORIZATION CHECK FOR ALL COMMANDS
                    ...
                    if (conf.MODE?.toLowerCase() === "no" && !isSuperUser) {
                        ...
                        try {
                            await adams.sendMessage(origineMessage, {
                                text: `🚫 *Access Denied*\n\nBot is in private mode. Only authorized users can use commands.\n\n_Contact bot owner for access._`,
                                ...createContext(auteurMessage, {
                                    title: "UNAUTHORIZED ACCESS",
                                    body: "Command execution blocked"
                                })
                            }, { quoted: ms });
                        } catch (err) {
                            console.error("Error sending unauthorized message:", err);
                        }
                        return; // BLOCK EXECUTION
                    }

                    // PHASE 2: CHECK SPECIFIC COMMAND RESTRICTIONS
                    const restrictedCommands = {
                        // OWNER/SUPERUSER ONLY COMMANDS (MOST RESTRICTED)
                        superUserOnly: [
                            'closegroup', 'opengroup', 'demote', 'promote', 'kick', 'add',
                            'antilink', 'antidelete', 'setpp', 'setbio', 'block', 'unblock',
                            'broadcast', 'eval', 'exec', 'restart', 'shutdown', 'update',
                            'setvar', 'delvar', 'getvar', 'ban', 'unban', 'mute', 'unmute',
                            'warn', 'unwarn', 'setname', 'setdesc', 'revoke', 'delete',
                            'clone', 'leave', 'join', 'setprefix', 'mode', 'autoread',
                            'autoreact', 'welcome', 'goodbye', 'antibot', 'antispam',
                            'settings', 'config', 'vars', 'env', 'deploy', 'heroku',
                            'system', 'server', 'logs', 'backup', 'restore', 'reset',
                            'clear', 'clean', 'purge', 'reboot', 'maintenance',
                            'debug', 'test', 'dev', 'admin', 'owner', 'sudo'
                        ],
                        // GROUP ADMIN ONLY COMMANDS (in groups) or SUPERUSER anywhere
                        adminOnly: [
                            'tagall', 'hidetag', 'totag', 'groupinfo', 'admins',
                            'online', 'inactive', 'gclink', 'resetlink', 'groupstats',
                            'members', 'mention', 'notify', 'announce'
                        ]
                    };
                    ...
                    if (restrictedCommands.superUserOnly.includes(com)) {
                        if (!isSuperUser) {
                            ...
                            try {
                                await adams.sendMessage(origineMessage, {
                                    text: `🚫 *Access Denied*\n\nCommand \`${com}\` is restricted to authorized users only.\n\n_Contact bot owner for access._`,
                                    ...createContext(auteurMessage, {
                    ...
                    if (restrictedCommands.adminOnly.includes(com)) {
                        if (!isSuperUser && (!verifGroupe || !verifAdmin)) {
                            console.log(`🚨 BLOCKED: ${auteurMessage} tried admin command ${com} - Not admin/superUser`);
                            store.logUnauthorizedAttempt(auteurMessage, com, "Admin-only command attempted by non-admin");
                            
                            try {
                                await adams.sendMessage(origineMessage, {
                                    text: `🚫 *Access Denied*\n\nCommand \`${com}\` requires admin privileges.\n\n${verifGroupe ? '_You must be a group admin to use this command._' : '_This command can only be used by authorized users._'}`,
                                    ...createContext(auteurMessage, {
                                ...
                                console.error("Error sending admin restriction message:", err);
                            }
                            return; // BLOCK EXECUTION
                        }
                    }
                    ...
                    if (verifGroupe) {
                        const botAdminRequired = [
                            ...
                            'mute', 'unmute', 'setname', 'setdesc', 'revoke'
                        ];
                        ...
                        if (botAdminRequired.includes(com) && !botIsAdmin) {
                            ...
                            try {
                                await adams.sendMessage(origineMessage, {
                                    text: `🚫 *Cannot Execute Command*\n\nI need admin privileges to execute \`${com}\`.\n\n_Please promote me to admin first._`,
                                    ...createContext(auteurMessage, {
                                        title: "BOT ADMIN REQUIRED",
                                        body: "Insufficient bot permissions"
                                    })
                                }, { quoted: ms });
                            } catch (err) {
                                console.error("Error sending bot admin message:", err);
                            }
                            return; // BLOCK EXECUTION
                        }
                    }
...
└─────────────────┈ ⳹`;
            ...
            if (connection === "close") {
                ...
                if (reason === DisconnectReason.badSession) {
                    ...
                    try {
                        await fs.remove(__dirname + "/bwmxmd");
                    } catch (e) {
